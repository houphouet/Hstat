...
#
#  Onglet migre en module Shiny (2e migration).
#  - mod_crosstab_ui(id)             : contenu de l'onglet (dans un tabItem)
#  - mod_crosstab_server(id, values) : logique serveur ; 'values' = reactiveValues
#    GLOBAL (fournit filteredData, dataMode, dbCon, dbTable, fullNrow).
#
#  Les identifiants input/output sont automatiquement namespaces par le module.
#  Cote UI chaque id est enveloppe par ns() ; cote serveur, les inputs/outputs
#  crees dynamiquement (renderUI) utilisent session$ns (alias 'ns').
################################################################################
