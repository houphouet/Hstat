...
#
#  8e migration. Ces deux onglets partagent un etat bidirectionnel
#  (modelList, testResultsDF, currentTestType, allPostHocResults...) : ils sont
#  donc migres ENSEMBLE dans un meme module pour preserver ce flux interne.
#  Lit les donnees partagees (filteredData, cleanData, data) via 'values'.
#  - mod_tests_ui(id)            : renvoie les DEUX tabItems (tests + multiple)
#  - mod_tests_server(id, values): 'values' = reactiveValues GLOBAL
################################################################################
                  ...
                  box(title = "Paramètres des tests", status = "danger", width = 12, solidHeader = TRUE,
                      fluidRow(
                        column(4,
                               ...
                               div(style = "background-color:#f3e9fb; border-left:4px solid #8e44ad; padding:10px; margin-top:12px;",
                        ...
                        column(4,
                               ...
                               div(style="background:#f3e5f5; padding:10px; border-radius:8px; margin-bottom:8px;",
                                   ...
                                   radioButtons(ns("chiSqMethod"), NULL,
                                                choiceNames  = list(
                    ...
                    fluidRow(
                      ...
                      column(4,
                             div(style = "background:#fff3e0; padding:14px; border-radius:8px; margin-bottom:10px;",
                                 ...
                                 radioButtons(ns("chiSqDataType"), NULL,
                                              choiceNames  = list(
                      ...
                      column(8,
                             tabsetPanel(id = "chiSqResultsTabs", type = "tabs",
                                         ...
                                         tabPanel(tagList(icon("table"), " Résumé global"),
                                                  ...
                                                  withSpinner(DTOutput(ns("chiSqGlobalResult")), color = "#1565C0"),
                                                  br(),
                                                  uiOutput(ns("chiSqInterpretation"))
                                         ),
                                       
                                         tabPanel(tagList(icon("layer-group"), " Groupes"),
                                                  value = "chiSq_groupes", br(),
                                                  withSpinner(DTOutput(ns("chiSqResumeTable")), color = "#1565C0"),
                                                  br(),
                                                  fluidRow(
                                                    column(4, downloadButton(ns("downloadChiSqExcel"), tagList(icon("file-excel"), " Excel complet"), class = "btn-success btn-block")),
                                                    column(4, downloadButton(ns("downloadChiSqCSV"),   tagList(icon("file-csv"),   " CSV groupes"),   class = "btn-info btn-block")),
                                                    column(4, downloadButton(ns("downloadChiSqCSVPaires"), tagList(icon("file-csv"), " CSV paires"),  class = "btn-info btn-block"))
                                                  )
                                         ),
                                       
                                         tabPanel(tagList(icon("code-branch"), " Paires Bonferroni"),
                                                  value = "chiSq_paires", br(),
                                                  withSpinner(DTOutput(ns("chiSqPairesTable")), color = "#1565C0")
                                         )
                             )
                      )
                    )
...
mod_tests_server <- function(id, values) {
  ...
  output$glmmRandomSelect <- renderUI({
    ...
    tagList(
      # 1) Variable(s) de groupement (effets croises si plusieurs)
  ...
  observeEvent(input$runChiSqTest, {
    ...
    # autant de "modalites" que de lignes -> combn() explose et l'app se fige.
